    ...
    <div class="card">
        ...
        <div class="card-body">
          <form action="{{ route('sellers.update', $shop->id) }}" method="POST">
                ...
                <div class="form-group row">
                    <label class="col-sm-3 col-from-label" for="password">{{translate('Registration Status')}}</label>
                    <div class="col-sm-9">
                        <select name="status" class="form-control">
                            <option value="0" {{ $shop->user->status == 0 ? 'selected' : '' }}>Pending</option>
                            <option value="1" {{ $shop->user->status == 1 ? 'selected' : '' }}>Accept</option>
                            <option value="2" {{ $shop->user->status == 2 ? 'selected' : '' }}>Reject</option>
                        </select>
                    </div>
                </div>
