...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $newest_products = Cache::remember('admin_newest_products', 3600, function () {
            return filter_products(Product::where('added_by', 'admin')->where('published', 1))->latest()->limit(12)->get();
        });
    ...
    public function product(Request $request, $slug)
    {
        ...
        $brand_id = $detailedProduct->brand_id;
        $weight = $detailedProduct->weight;
        $seller_ids = DB::table('seller_products')->where('product_id', $detailedProduct->id)->pluck('seller_id')->toArray();
        $sellers = User::whereIn('id', $seller_ids)->get();
        ...
        if ($detailedProduct != null && $detailedProduct->published) {
            ...
            return view('frontend.product_details', compact('detailedProduct', 'product_queries', 'total_query', 'reviews', 'review_status', 'sellers'));
        }
        abort(404);
    }
    ...
    {   
        ...
        if($request->filled('seller_id')){
            $seller_product = \DB::table('seller_products')->where('seller_id', $request->seller_id)->where('product_id', $request->id)->first(); 
        }
        $str = '';
        $quantity = 0;
        $tax = 0;
