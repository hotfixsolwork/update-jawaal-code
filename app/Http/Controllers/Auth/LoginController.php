    ...
    */
    ...
    public function login(Request $request)
    {
        ...
        if($request->filled('login_type')){
            if($request->input('login_type') == 'seller'){
               $credentials['user_type'] = 'customer';
            }
            
            elseif($request->input('login_type') == 'supplier'){
               $credentials['user_type'] = 'seller';
            }
            
            elseif($request->input('login_type') == 'admin'){
        ...
        if(Auth::attempt($credentials, true)){
            ...
            if (auth()->user()->user_type == 'admin' || auth()->user()->user_type == 'staff') {
                ...
                if(auth()->user()->status !== 1){
                    $user = auth()->user(); 
                    auth()->logout();
                    return redirect()->signedRoute('seller.registration.status', ['user_id' => $user->id]);
                }
                return redirect()->route('seller.dashboard');
                 ...
                 // check if user is verified
                    if(auth()->user()->status !== 1){
                        $user = auth()->user(); 
                        auth()->logout();
                        return redirect()->signedRoute('registration.status', ['user_id' => $user->id]);
                    }
