...
if (!function_exists('cart_product_price')) {
    function cart_product_price($cart_product, $product, $formatted = true, $tax = true)
    {
        ...
        // for checking if seller added product then update seller price
        $owner_id = $cart_product['owner_id'];
...
if (!function_exists('home_price')) {
    function home_price($product, $formatted = true, $seller_id = null)
    {
        ...
        if(!empty($seller_id)){
            $seller_product = \DB::table('seller_products')->where('seller_id', $seller_id)->where('product_id', $product->id)->first();
            if($seller_product){
                $highest_price = $seller_product->price;
            } 
        } 
...
if (!function_exists('addon_is_activated')) {
    ...
    {
        $addons = Cache::remember('addons_new', 86400, function () {
            return Addon::all();
        });
...
function sendSMSOTP($phone)
{
    ...
    session()->put('sms_verification_opt', $otp);
    ...
    //     'userName' => 'rewaq.alnour', // username for the account in Msegat.com
    //     'numbers' => $phone,
    //     'userSender' => 'auth-mseg', // sender name, should be activated from Msegat.com
    //     'apiKey' => '72a73141fc4391ca00c3c88f3c2805ff', // apiKey associated with the account
...
function sendEmailOTP($email)
{
    ...
    Mail::send('emails.verification_otp_email', ['otp' => $otp], function ($m) use ($email) {
            $m->from(config('mail.username'), config('mail.from.name'));
            $m->to($email)->subject('Verification OTP!');
        });
