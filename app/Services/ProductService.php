...
class ProductService
{
    public function store(array $data)
    {
        ...
        if ($collection['rams'][0] != null) {
            foreach (json_decode($collection['rams'][0]) as $key => $ram) {
                array_push($rams, $ram->value);
            }
        }
        
        if ($collection['storage'][0] != null) {
            foreach (json_decode($collection['storage'][0]) as $key => $st) {
                array_push($storage, $st->value);
            }
        }
        
        $collection['tags'] = implode(',', $tags);
        $collection['rams'] = implode(',', $rams);
        $collection['storage'] = implode(',', $storage);
    ...
    public function update(array $data, Product $product)
    {
        ...
        if ($collection['storage'][0] != null) {
            foreach (json_decode($collection['storage'][0]) as $key => $sto) {
                array_push($storage, $sto->value);
            }
        }
